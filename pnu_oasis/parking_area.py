...
map = open("D:\\git_gwangjonghan\\pnu_oasis\sido_map.csv", "r", encoding='UTF8')
k = 0
while True:
    line = map.readline()
    if not line: break
...
f = open("중간과정.csv", "w")
index = 0
for section_num in range(0,len(busan_dong)):
    for public_or_private in range(2):
        if public_or_private == 0:
            ...
            pubpri = "민영"
        try:
            url = "http://map.naver.com/?query="+str(busan_dong[section_num])+" 주차장"
